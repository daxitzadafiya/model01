...
type Props = {
  ...
  /** Fade/slide in content after loading (e.g. async similar projects on detail page). */
...
export const ProjectsCarousel: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    setEntryRevealed(false)
  }, [animateEntry, showLoading, hasProjects, projects.length])
  ...
  useLayoutEffect(() => {
    if (!hasProjects || showLoading) return
    measureSlideStride()
    if (!animateEntry) {
      activateRevealElements(carouselRef.current)
      activateRevealElements(sectionRef.current)
    }
  }, [animateEntry, hasProjects, showLoading, projects.length, measureSlideStride, cardsPerView])
  ...
  }, [measureSlideStride, total, cardsPerView, hasProjects, showLoading])

  if (!showLoading && !hasProjects && !showWhenEmpty) return null
  ...
  return (
    <section ref={sectionRef} className={cn(`py-16 md:py-24 ${bgClass}`, className)}>
      ...
      >
        ...
        {hasProjects && !showLoading && canSlide && (
      ...
      {showLoading ? (
        ...
        >
          <div
            ref={trackRef}
            className={cn('flex items-stretch ease-in-out', isDragging ? '' : 'transition-transform')}
            style={{
              gap: `${GAP_PX}px`,
          ...
          >
            {displayProjects.map((project, idx) => {
              const cardKey = project.id ?? project.reference ?? String(idx)
              return (
                <div key={cardKey} className="shrink-0 h-auto" style={{ width: cardWidth }}>
                  <ProjectCard
                    project={project}
